...
PROFILE_SYSTEM = (
    ...
    "「当前重心」类内容不进画像——月计划是唯一事实源，由晚间报告承载。"
    "输出修订后的画像正文（不含标题、不含手写区、不含 user 标记），"
    "从第一个 ## 小节开始，不要输出任何其他文字。"
)

# 画像允许的小节（结构白名单：GLM 只能在这五节里写）
ALLOWED_SECTIONS = ("## 节律与习惯", "## 规划校准",
                    "## 沟通偏好", "## 常见滑落模式", "## 修订记录")
MAX_BULLET_CHARS = 120

# 周报里可作画像证据的统计区块（v13 编号：一滑落/四节律/六用量；硬节点/inbox/
# 待定问题/三件事含具体任务与私务，绝不进画像的证据源）
SAFE_WEEKLY_SECTIONS = ("## 一、", "## 四、", "## 六、")


def sanitize_weekly_body(body: str) -> str:
    """只保留周报的统计区块（一/四/六），任务性内容（二/三/五/摘要/三件事）剔除。"""
    keep: list[str] = []
    cur_ok = False
