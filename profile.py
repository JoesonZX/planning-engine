...
PROFILE_SYSTEM = (
    ...
    "只陈述可验证的行为模式（当前重心/节律/估时校准/滑落模式/沟通偏好），"
    "每条一行、行尾带证据标注（如「W37·滑落区」「config·时间线」）；"
    "无新证据的条目原样保留，不要改写；禁止推测或评价用户的情绪、心理、感情状态；"
    "禁止任何评价性、教练式语气。输出修订后的画像正文（不含标题、不含手写区、"
    "不含 user 标记），从第一个 ## 小节开始，不要输出任何其他文字。"
)
...
def update_profile(root: Path, cfg: dict, api_key: str | None,
    ...
    revised = _glm_revise(handwritten, old_agent, weekly_body,
    ...
    new = f"{TITLE}\n\n{USER_START}\n{handwritten.rstrip()}\n{USER_END}\n\n{revised.strip()}\n"
    # 守卫二：行数上限（聊天上下文每文件 6000 字符 cap 的余量保证）
    if new.count("\n") + 1 > MAX_LINES:
